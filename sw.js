/* Shadow Hook — service worker  (auto-updating)
   ...
   Why this makes old versions go away:
     ...
     • Bumping the number below also wipes every old cache on activate, so nothing
       stale can survive.
   ──────────────────────────────────────────────────────────────────────────── */
const CACHE = 'shadowhook-v80';  // ← bump this each time you deploy an update  (v80: #eofguard — NEVER LIGHT A BLOCK FROM A CUT-OFF PAGE. THE ALARMS, Sep 18 2:11 PM, a first visit: 'Uncaught SyntaxError: Unexpected end of input @ :391' out of insertBefore — the yard-map blocks in the file are whole (every one parses, none is 391 lines), that phone's copy of index.html was not: a one-bar download stopped partway, the browser ran what it had, and the idle door re-inserted a block sliced mid-line — with everything after the cut missing anyway. The file now ends in a one-line sentinel; if it never ran the parser never reached the end, so shkChunk holds the maps, files the cut under the dev log, and fetches the page again once per session when the signal is back. Build stamp sep18eof.  ── v79: #thinstop — THE STOP IS THE STOP, EVEN IN A THIN LETTER. A predicted stop that falls inside the rest zone (K→T worked last night, the call says the board reaches K on the flops) carried both .stop and .rest, and .rest won the cascade: the base skin gave back only the white text, and the daylight and arcade skins' own .rest rules outranked even that, so the stop drew as a blocked-off hatched cell with STOP written under it. The stop's look now wins in all three skins, with the hatch kept as a faint overlay so the cell still reads thin. Build stamp sep18thin.  ── v78: #jackpotback — THE SHOW HAD NO STAGE. The jackpot overlay's markup and CSS (#jackpot, #jpAmount, #jpReason, #jpClaim, .jp-coin) were gone from the page while the code that plays on them stayed; jackpot() bailed on the missing element, so every win since — the mint, the first-to-log-the-board spin — was silent, and awardJackpot credits quietly on the assumption the show is the feedback, so a winner saw nothing but the header number move. The stage now builds itself the first time it is needed, styles injected once; COLLECT or a tap on the dark closes it, and it folds on its own after eight seconds. Build stamp bumped to sep18mint so THE ALARMS can tell this build from the last.  ── v77: SIX PATCHES, ALL IN index.html (sep17). ① #slipmend — a phone still on the pre-#slipvault build strips the photo mark off every row older than the ten newest on each save; the mark is restored from the phone's own store wherever the log is rebuilt from disk. ② #slipkeep — the server's echo of the log could land over a phone's copy before the store had confirmed a picture, leaving a mark with nothing behind it (the report: two slips scanned on the phone, viewer said they were on the other phone); unconfirmed bytes now ride onto the incoming copy until the store has them; photos are recorded as raw bytes, not Blobs; a force-closed store connection reopens; store failures report to DEVLOG under slipstore; rescanning a slip already in the log puts the photo on that entry instead of refusing. ③ #camlock — a second SCAN tap during the sensor wake built a duplicate viewfinder with no stream and no handlers: a black screen only a kill could close; the overlay now goes up before getUserMedia, an opening lock swallows the re-tap, every lookup is scoped to its own overlay, a late stream is stopped, the preview resumes after a background nap. ④ #slipquiet — a dimmed 📷 toasts one line instead of opening a broken viewer. ⑤ #firingline — 'You're in the firing line' buzzed a card nineteen letters out because the test was distance against the 75th-percentile stop plus slack, which a spike in the distribution fools; it is now P(board reaches your card) ≥ 0.20, held ten seconds and re-read before it fires, and the 'Alerts on' confirmation no longer eats the day's real alert. ⑥ #mintsoon — THE BANK's ⬡ packs are gated behind MINT_OPEN (or km_flags.mint = 'open'); shut, they draw dimmed under a COMING SOON card and a tap never calls bucks-till; the INSUFFICIENT FUNDS sheet's own $1.99/$4.99/$9.99 packs are retired in favour of one Open the Bank button. NEEDS ONE PASTE — alter table km_flags add column if not exists mint text; then update km_flags set mint='open' where id=1 opens the mint once bucks-till runs on the live Stripe key.  ── v76: THE LESSON THAT SENT SSA TO TOYOTA. Two members fixed a Toyota slip — the RORO dock prints SSA, its stevedore, over LB 83 — and the reader stored "SSA → TOYOTA" as a shared lesson. At two votes a lesson overrode a clean read, so from Sep 7 every slip whose operator line read plainly SSA — Matson, Zim, both Cosco yards, the Tower, the Steel Dock — sealed at Toyota RORO with a gate route to Pier B. #lessonguard, all in index.html (sep11d): a lesson now FILLS a blank or corrects a GUESS and never overrides a name read plainly off its own line, at any vote count, self included — same rule for job codes (the whole-slip sweep marks its result catFuzzy; a code read under the header stands). A lesson keyed on an operator's OWN name is never applied and never taught. Berth lessons whose truth is a dock's SPAN ("LB 88 96") are ignored and no longer recorded — they were the seal writing the span back, and they broke berthCandidates on every slip they touched. LESSONS_REVOKED names the SSA → TOYOTA row; revoked rows are purged from each phone's cache on load and skipped on every pull, so the fix reaches phones whatever the server still holds. And when a clean operator contradicts the berth (SSA over LB 83), the ambiguity prompt now names the berth's dock too, so the casual isn't shown six SSA yards and no Toyota — the friction that taught the poison in the first place. NEEDS ONE PASTE — LESSONS-REVOKE.sql in Supabase deletes the two server rows (and, optionally, the operator-name and span-berth rows the reader no longer uses). Dockets already sealed at Toyota since Sep 7 are not rewritten; those hands fix their rows by hand, and doing so now teaches nothing wrong.  ── v75: CLIENTS CAN REACH THE MAKERS, AND THE FREEZE ALARM STOPS CRYING WOLF. FOUR THINGS LANDED. ① THE SHOP WIRE — until now a member who wanted to ask the maker of a piece a question had nowhere to ask it; the only wire in the building ran vendor ↔ Keymaster. There is now a line between the two people an order is actually about. 💬 ASK THE MAKER sits under the buy button on any piece that belongs to a vendor (house gear with no maker has no button) for the questions that come before money — sizing, materials, can you do it in red. 💬 MESSAGE THE MAKER sits on each paid order under MY ORDERS, one button per maker on it, for the questions that come after — any chance it ships this week, wrong size can I swap. Both land in 💬 INBOX, a fourth shelf beside PRODUCTS · SERVICES · MY ORDERS, with a red count on unread replies and an orange dot on the Armory tab from anywhere in the app. The same member asking the same maker about the same piece lands in the thread they already have, not a new one — a wire that spawns a fresh thread per tap is a wire nobody reads. Signing in is required to write, because an unclaimed card is not a person anyone can answer. THE BROWSER ONLY EVER READS these tables: every write goes through a function that checks who is asking, so a member cannot open a thread on somebody else's order or on a piece that is not that maker's, and cannot write on a closed one. Text only, 2,000 characters, no photos on this wire. NEEDS ONE DEPLOY — run SHOP-WIRE.sql in Supabase, or the buttons are there and nothing sends. ② THE FREEZE ALARM WAS LYING. THE ALARMS filled with 'main thread stalled' reports of several thousand seconds, and a stall measured in HOURS is not a stall — it is a phone that went to sleep with the app open. The old watchdog compared two wall-clock readings and called the gap a freeze, so every pocket, every overnight, every locked screen wrote an alarm; the real jams were buried under them, which is worse than no watchdog at all. It now requires the page to be VISIBLE for the whole window, agreement between the wall clock and the monotonic clock (they diverge exactly when a device suspends), a 1.5-second grace after any wake event, a 30-second ceiling above which the answer is 'the phone slept' rather than 'the app hung', and it reports once per screen instead of once per tick. On Chrome it also takes the browser's own longtask reading as the second witness. A stall that survives all of that is a real one. ③ MY DECK — a vendor or a crowned ambassador signed into the app sees their own deck's door in the Armory where the recruitment poster used to be, instead of being pitched a job they already have. ④ KEY HOLD ON THE PAY LIST — two job codes added to the catalog every phone starts from, the log form's code list and the Drop Log: HOLD (Hold Man, basic tier, base rate, shift default) and KH (Key Hold, key tier, base + $6.50/hr clerk-key differential, paid as a 10-hour day). That prices Key Hold as the top job a casual gets sent to — above UTR because of the differential, and a 10-hour day besides. Both Key Hold numbers are a read of what the Keymaster described, not the PMA table, which does not name the job; correcting either one in the log form when a KH docket is sealed sticks on that phone.  ── v74: THE TILL MARKS ASK THE ORDERS TABLE. v73 gated Till Ringer and the Patron's Key on a footprint the phone left on its way to Stripe — better than the bare ?armory_paid parameter it replaced, but still the member's own word. The stripe-webhook turned out to already write the real answer: armory-checkout creates the orders row, checkout.session.completed flips payment_status to 'paid', and `code` is exactly what comes back in the return URL. So the question is now the honest one — is there a paid order with this code, and is it mine — and the server answers it. TIMING MATTERS AND IS HANDLED: Stripe redirects the member back immediately while the webhook is a separate call that may not have landed, so a legitimate return can arrive on a row still reading 'pending'. Refusing there would punish the honest case, so it re-asks twice as the row settles; an order that never turns paid never awards anything. IF THE TABLE CANNOT BE READ — no session yet, or RLS does not let a member see their own orders — it falls back to the v73 footprint rather than refusing, because "cannot check" and "checked and it is false" are different answers and must not be treated the same. Verified against every verdict: paid awards, no-such-order refuses, forever-pending refuses, unreadable-with-footprint awards, unreadable-without refuses, and a return with no code at all refuses. On a clean device a hand-typed ?armory_paid now awards nothing and writes nothing to disk. The #armlog book line stays deleted; it returns when a webhook writes bucks_book.stripe_session and the server is the one saying it.  ── v73: THE BOOK STOPS TAKING THE PHONE'S WORD ON REAL MONEY. handleReturn fired on the presence of ?armory_paid in the URL and nothing else — no order lookup, no session check, no nonce. Loading index.html?armory_paid=1&code=AAA three times awarded Till Ringer and the Patron's Key AND wrote three lines into bucks_book saying "armory order … — paid by card". The 60 ⬡ were never the problem; the lines were. bucks_book is the record THE BOOKS audits, and this was putting unverifiable claims of card payments into it from a text field. That #armlog line is removed outright — the server records the order or nothing ships, so a second, weaker copy written by the phone added no information and forged everything. It comes back the day a Stripe webhook fills bucks_book.stripe_session, and then it will be the server saying it. Checked first: that column exists and has never been written to, which is why no client-side check could have been honest. The two marks are now gated on a one-use marker that pay() drops on the way out to the checkout and handleReturn consumes on the way back, six-hour expiry, device-local and deliberately outside the sync. Not proof — someone who really means it can open the till, cancel, and craft the URL — but it is the difference between typing eleven characters and walking through a checkout on purpose. A hand-typed return still shows the PAID message and still clears the cart, because a member who genuinely paid on another phone should not be told they did not. Verified: three hand-typed returns award nothing and move no tally; three real ones award both marks and a tally of 3; a marker seven hours old counts for nothing. THE ACTUAL WALL is the webhook — this is the honest fence until it exists.  ── v72: A TYPED HOUR FIGURE HAS TO BE WITHIN REACH OF THE RECORD. The hours sheet took any number between 0 and 20,000 and wrote it straight into the book — and the book drives the pay ladder, the Race AND the marks wall. A hand with 100 hours could type 8,000 and light nine hour-marks, Century Club through The Vault, in about ten seconds: 270 ⬡ out of a text field, and a lane near the top of the Race they had not worked for. This does NOT lock the typed figure — #hoursown is right that the posting is a snapshot of one day with a closed roster and the hand holding the stub can be right when the snapshot is not, so correcting the record has to stay possible. What stops being possible is inventing it. The ceiling is what the Order's own record says this card had, plus what they could physically have worked between the posting cutoff and the paid-through date they are claiming: ten hours a day, every day, no days off — seventy a week sustained for the whole stretch, which nobody does. Plus a flat 100-hour tolerance so a genuine discrepancy is never blocked by arithmetic, and the allowance is CAPPED at 1,000 so the guard does not quietly loosen itself every day the posting sits unrefreshed. Work done since the cutoff was never supposed to live in this figure anyway — sealed dockets add it on top by themselves. Checked against the whole posting: 0 of 5,510 hands have a posted figure above their own ceiling, so no honest hand is blocked. A card not on the posting has no figure to check against and is held to tolerance plus allowance; a real book bigger than that is a conversation with the Keymaster, not a text field, and the message says so.  ── v71: THE MARKS WALL STOPS PAYING TWICE. A mark paid PAY.MARK ⬡ off the in-memory flag while only the disk write was deduped against the stored list, so any moment the two came apart the mark paid again and wrote nothing — no trace on the device, and the badge key byte-identical before and after. Four routes reached that state with nobody touching a console: settle() rebuilt the flags from a MISSING, EMPTY or SHORTER server row and zeroed all 112 first; the cloud apply() writes String(v!=null?v:'') so a null row lands as an empty string; both boot restores JSON.parse that empty string, throw, and restore ZERO flags; and settle()'s badge branch is skipped while the card reads "anon" although apply() has already written the server's list to disk. settle() only repaints, so the wipe was silent and went off on the NEXT checkMarks() — a docket seal, an hours save, a strike, or the next open. That is the "I sealed one docket and got forty marks", up to 107 × PAY.MARK a go, repeatable. THREE CHANGES, all in index.html: unlockBadge now reads the STORED LIST, writes it, and only then pays — an unreadable or empty value heals the key from the wall and pays for nothing, because when a mark can't be proved unpaid the honest default is silence, not money; settle() unions instead of replacing, since a mark is monotonic and a sync may only ever ADD one, exactly the way the dockets branch beside it already merges; and both boot restores refuse to read an empty string as an empty list, the way reloadAcctBucks already did. And the root of it: the badges key was doing two jobs at once — it was the WALL (display state, syncs between a member's phones, replaceable by any of them) and it was also the PAYMENT RECORD, and conflating those is why a display problem could mint currency. They are two keys now. shadowhook_badges_<card> stays the wall and syncs as before. shadowhook_markspaid_<card> is the record of what this card has actually been paid for: device-local, excluded from the cloud mirror in mine() exactly like the credits key, and nothing else decides whether ⬡ moves. A nulled server row, a shorter list from a second phone, an empty string — none of them can reach it. It is seeded from the wall the first time a card runs this build, and again on every path that reassigns ACCT_ID, because K() repoints at a different card's keys and an absent payment record would otherwise read as "nothing paid" and buy the whole wall a second time. A card whose payment record reads as junk pays for NO mark for the rest of that session, not just the first one — the rebuild makes the key readable again, so guarding only the first mark left the other 89 of a re-derived wall paying out normally, a 2,640 ⬡ hole in the middle of the fix for a 3,210 ⬡ hole. The cost is one session where a genuinely new mark records without paying; the member earns it again tomorrow against a clean key, and a re-minted wall cannot be taken back. The payout reason now carries the mark's NAME — "new mark earned · Night Owl" — so the book can finally tell a legitimate catch-up from a repeat. NOT FIXED YET, say it plainly: the record of what has been paid still lives in localStorage, so clearing site data still mints. The wall is a marks_awarded row with a unique constraint and an apply_bucks that refuses a mark delta without it. No rule thresholds were touched — the loose ones are still loose.  ── v70: THEIR BOOK. The statement a member reads in THE BANK now opens on ANY hand's page in the Command Center — tap a roster row and the lines are right under their stack, newest first, each one saying which record it came from. Two records are merged: buck_ledger, written server-side, which carries the running balance after every move and marks a pack bought with real money; and bucks_book, written by the app from the phone once the bank confirms. A line that exists only in the book is labelled 'book only', because the phone wrote it and the phone can lie — that distinction is the whole point. Above the lines sits the reckoning: what the card says, what the lines add up to, and the gap. If those two numbers disagree, ⬡ moved without writing a line, and no explanation makes that innocent. Riding along: THE BOOKS, above the roster — nobody reads 132 statements by eye, so every hand whose book has something to answer for surfaces in one list, worst first, with the reason named. DRIFT (card and lines disagree) · CHAIN BREAK (a ledger row's running balance doesn't follow the one before it) · NO LINES (a stack above zero with nothing written down) · CHURN (sealing dockets and striking them straight back — the #fix2(bucks) hole being walked on) · BURST (credits piled inside sixty seconds) · HOT DAY · BIG CREDIT · BLANK REASON (apply_bucks called by hand) · CARRY. A paid pack never lights a flag, and a flagged hand carries a ⚠ on the roster line so the list and the page can't disagree. Read-only: nothing here moves ⬡. NEEDS ONE DEPLOY — run MEMBER-BOOK.sql in Supabase, or the Command Center says so out loud instead of showing an empty book. No push changes; the v66 handlers stand.)
