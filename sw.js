/* Shadow Hook — service worker  (auto-updating)
   ...
   Why this makes old versions go away:
     ...
     • Bumping the number below also wipes every old cache on activate, so nothing
       stale can survive.
   ──────────────────────────────────────────────────────────────────────────── */
const CACHE = 'shadowhook-v68';  // ← bump this each time you deploy an update  (v68: THE LOOKOUT — true closed-app push: the board's movement buzzes every hand within 3 letters of the next opening card (hall-line worker sends it; PUSH-SETUP.sql + the VAPID keys), and a paid Armory order pings the Keymaster's phones (push-order function + orders webhook). Also riding this build: the truth-telling boot loader (crest shows ONCE, bar measures the real load); verified hours = the official posting + sealed dockets, one figure everywhere; the marks wall audited — all 112 wired, daylight earned/sealed contrast, NaN-proof date math; the gate knows name ORDER (ROSTER_ORD); ONLINE roster carries real handles; Command Center grew the Mailbag, the Traffic (PULSE.sql), and the Stockroom — vendor gear waits for the Keymaster's word (VENDOR-DESK.sql), with returns & warranty on every product page. This sw.js needed NO push changes — the v66 handlers already carry the Lookout's payloads.)
